...
INSERT INTO product_units
	VALUES (24, 'Sweet Corn', '2 Ear', 1, 'unit', CURRENT_TIMESTAMP)
	...
	SET current_quantity = COALESCE((
		...
		FROM (
			SELECT
					...
					ROW_NUMBER() OVER (PARTITION BY vendor_inventory.product_id 
					ORDER BY vendor_inventory.market_date DESC) AS row_num
				FROM vendor_inventory
		) AS ranked_inventory
		
		--where statement
		WHERE ranked_inventory.product_id = product_units.product_id
		AND ranked_inventory.row_num = 1
	), 0);
